...
export class FeedCreate extends OpenAPIRoute {
  ...
  async handle(c: AppContext) {
    ...
    if (content.length > 2000) {
      return Response.json(
        { error: 'Bad Request', message: 'Content must be less than 2000 characters', status: 400 },
        { status: 400 }
      )
    }
    ...
    if (!post) {
      return Response.json(
        { error: 'Internal Server Error', message: 'Failed to create post', status: 500 },
        { status: 500 }
      )
    }
    ...
    const postWithUser = {
      ...
      images_array: images || [],
      user: toUserPublic(user, { username: user.username, display_name: user.display_name, avatar_url: user.avatar_url }),
    }
