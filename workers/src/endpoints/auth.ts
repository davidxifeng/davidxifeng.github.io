  ...
  async handle(c: AppContext) {
    ...
    if (!isValidEmail(email)) {
      return Response.json(
        { error: 'Bad Request', message: 'Invalid email format', status: 400 },
        { status: 400 }
      )
    }
...
export class AuthRegister extends OpenAPIRoute {
  ...
  async handle(c: AppContext) {
    const data = await this.getValidatedData<typeof this.schema>()
    const { email, username, password, verification_code, display_name } = data.body
    ...
    if (!passwordValidation.valid) {
      return Response.json(
        { error: 'Bad Request', message: 'Weak password', status: 400, details: passwordValidation.errors },
        { status: 400 }
      )
    }
    ...
    if (!isValidVerificationCode(verification_code)) {
      return Response.json(
        { error: 'Bad Request', message: 'Invalid verification code format', status: 400 },
        { status: 400 }
      )
    }
    ...
    if (!verificationRecord) {
      return Response.json(
        { error: 'Bad Request', message: 'Invalid or expired verification code', status: 400 },
        { status: 400 }
      )
    }

    // Check if email or username already exists
    const existingUser = await c.env.DB.prepare('SELECT id FROM users WHERE email = ? OR username = ?')
      .bind(email, username)
      .first()

    if (existingUser) {
      return Response.json(
        { error: 'Conflict', message: 'Email or username already registered', status: 409 },
        { status: 409 }
      )
    }
    ...
    const now = getCurrentTimestamp()
    const sanitizedDisplayName = display_name ? sanitizeInput(display_name) : username

    await c.env.DB.prepare(
      'INSERT INTO users (id, email, username, password_hash, display_name, role, email_verified, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
    )
      .bind(userId, email, username, passwordHash, sanitizedDisplayName, 'user', 1, now, now)
      .run()
    ...
    if (!user) {
      return Response.json(
        { error: 'Internal Server Error', message: 'Failed to create user', status: 500 },
        { status: 500 }
      )
    }
...
export class AuthLogin extends OpenAPIRoute {
  ...
  async handle(c: AppContext) {
    ...
    if (!user.email_verified) {
      return Response.json(
        { error: 'Forbidden', message: 'Please verify your email before logging in', status: 403 },
        { status: 403 }
      )
    }
