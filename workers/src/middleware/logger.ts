...
function colorLog(
    ...
    title: string,
    data: any,
    color: string = colors.brightCyan
) {
    console.log(
    ...
    console.log(
        `${color}${colors.dim}${'='.repeat(title.length + 10)}${colors.reset}`
    )
}
...
export function createLogger(options: LoggerOptions = {}) {
    ...
    return async (c: Context<{ Bindings: Env }>, next: Next) => {
        ...
        // 检查是否应该记录此路径
        const shouldLog =
            ((opts.includePaths && opts.includePaths.length === 0) ||
                opts.includePaths?.some(p => path.startsWith(p))) &&
        ...
        if (opts.logHeaders) {
            ...
            if (allHeaders && typeof allHeaders === 'object') {
                ...
                requestInfo.headers = Object.entries(allHeaders).reduce(
                    (acc, [key, value]) => {
            ...
            try {
                ...
                if (contentType.includes('application/json')) {
                    ...
                    if (bodyStr.length > maxLength) {
                        requestInfo.body =
                            bodyStr.substring(0, maxLength) + '...[truncated]'
                    } else {
