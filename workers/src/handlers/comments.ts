...
export async function createComment(c: Context<{ Bindings: Env }>): Promise<Response> {
  try {
    ...
    if (content.length > 2000) {
      return Errors.badRequest('Comment content must be less than 2000 characters')
    }
 ...
 */
export async function updateComment(c: Context<{ Bindings: Env }>): Promise<Response> {
  try {
    const user = getCurrentUser(c)
    if (!user) {
      return Errors.unauthorized()
 ...
 */
export async function deleteComment(c: Context<{ Bindings: Env }>): Promise<Response> {
  try {
    const user = getCurrentUser(c)
    if (!user) {
      return Errors.unauthorized()
 ...
 */
export async function toggleCommentLike(c: Context<{ Bindings: Env }>): Promise<Response> {
  try {
    const user = getCurrentUser(c)
    if (!user) {
      return Errors.unauthorized()
