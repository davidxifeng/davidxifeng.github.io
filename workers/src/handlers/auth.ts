 ...
 */
export async function requestVerificationCode(c: Context<{ Bindings: Env }>): Promise<Response> {
  try {
...
export async function register(c: Context<{ Bindings: Env }>): Promise<Response> {
  try {
    ...
    const { email, username, password, verification_code, display_name } = body
 ...
 */
export async function getProfile(c: Context<{ Bindings: Env }>): Promise<Response> {
