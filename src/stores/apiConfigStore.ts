...
export const useAPIConfigStore = create<APIConfigState & APIConfigActions>()(
  persist(
    immer((set, get) => ({
      ...
      effectiveBaseURL: (API_PROVIDERS.find(p => p.id === DEFAULT_CONFIG.selectedProviderId) || API_PROVIDERS[0])
        .baseURL,
      effectiveModel:
        DEFAULT_CONFIG.model ||
        (API_PROVIDERS.find(p => p.id === DEFAULT_CONFIG.selectedProviderId) || API_PROVIDERS[0]).defaultModel ||
        'local-model',
      ...
      setProvider: (providerId: string) =>
        set(state => {
          ...
          const baseURL =
            newProvider?.id === 'custom' && state.customBaseURL ? state.customBaseURL : newProvider?.baseURL || '';
      ...
      setModel: (model: string) =>
        set(state => {
          console.log('setModel called - changing from', state.model, 'to', model);
          state.model = model;
      ...
      loadAvailableModels: async () => {
        ...
        console.log('Starting to load models for provider:', currentState.selectedProviderId);
        ...
        try {
          ...
          set(state => {
            ...
            if (models.length > 0 && (!state.model || !models.includes(state.model))) {
              console.log('Auto-selecting first model:', models[0], 'previous model was:', state.model);
              state.model = models[0];
            } else {
              console.log('Keeping existing model:', state.model, 'available models:', models);
            }
          });
        } catch (error: any) {
